 ...
 * (docs/decisions.md D1). Nothing here needs USB.
 *
...
class FoldPrivilegedService() : IFoldPrivileged.Stub() {
    ...
    private val launcher by lazy { RemoteLaunch { message -> runCatching { callback?.onLog(message) } } }
    override fun launch(intent: android.content.Intent, userId: Int, from: android.graphics.Rect, screenRadius: Float): String =
        launcher.launch(intent, userId, from, screenRadius)
    override fun launchRoute(): String = launcher.route

    // Fold8Duo (WP-58): the platform's own Full screen for one app, switched as the shell user (AppCompatOverrides.kt).
